...
namespace CreativeSpore.SmartColliders
{
    ...
    public class PlatformCharacterController : MonoBehaviour
    {
        ...
        protected void DoClimbing()
        {
            ...
            if (m_isClimbing)
            {
                if (GetActionState(eControllerActions.Jump) || GetActionState(eControllerActions.PlatformDropDown))
                {
                    m_isClimbing = false;
                    m_isGrounded = true;
                    return;
                ...
                bool isLadder = (m_currentClimbingCollider is BoxCollider2D) && ((BoxCollider2D)m_currentClimbingCollider).size.x < m_ladderWidthFactor * m_smartCollider.Size.x;
            ...
            {
                if (climbingColliderAbove != null && !GetActionState(eControllerActions.Jump))
                {
